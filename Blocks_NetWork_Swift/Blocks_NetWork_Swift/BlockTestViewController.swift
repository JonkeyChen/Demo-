...
class BlockTestViewController: UIViewController {
    ...
    private func networkRequest() {
        ...
        NetworkManger.GET(withUrl: urlStr, paramters: paramter, success: { (respond : [String : AnyObject]?) in
            print(respond)
            }, failure: { (error) in
        ...
        NetworkManger.POST(withUrl: urlStr_Post, paramters: param_Post, success: { (respond : [String : AnyObject]?) in
            print(respond)
        }) { (error) in
            print(error?.localizedDescription)
        }
